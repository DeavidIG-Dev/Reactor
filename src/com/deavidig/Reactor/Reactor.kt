...
object Reactor {
	...
	enum class JReactorErrorType(val message : String) {
	...
	enum class JReactorInfoType(val message : String) {
		ArgumentValue("Argument Type"),
	}

	fun setReactorError(errorType: JReactorErrorType, message: String): Nothing {
		println("$ANSI_RED[Reactor.JReactor - 1.0] ${errorType.message} (Instruction Error): $message$ANSI_RESET")
		exitProcess(1)
	}

	fun setReactorError(errorType: Array<JReactorErrorType>, message: String): Nothing {
		println("$ANSI_RED[Reactor.JReactor - 1.0] ${errorType.joinToString(" & ") { it.message }} (Instruction Error): $message$ANSI_RESET")
		exitProcess(1)
	}

	fun setReactorInfo(infoType: JReactorInfoType, message: String): Unit {
